      ...
      <div className="space-y-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 pl-11 pr-10 py-3 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition dark:text-slate-100 dark:placeholder-slate-500"
          />
          {query && (
            <button
          ...
          {query && (
            <div className="absolute z-10 mt-2 w-full bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 max-h-96 overflow-y-auto">
              {buscando ? (
                <p className="text-sm text-slate-400 dark:text-slate-500 p-4">Buscando...</p>
              ) : !hayResultados ? (
                <p className="text-sm text-slate-400 dark:text-slate-500 p-4">No encontramos nada con "{query}".</p>
              ) : (
                <div className="p-2">
                  {resultados.negocios.length > 0 && (
                    <div className="mb-2">
                      <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide px-2 py-1">
                        Negocios
                      </p>
                      {resultados.negocios.map((n) => (
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2"
                        >
                          <Building2 size={16} className="text-red-600 shrink-0" />
                          <span className="text-sm text-slate-700 dark:text-slate-200">{n.nombre_negocio}</span>
                        </button>
                      ))}
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      {resultados.productos.map((p) => (
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-between gap-2"
                        >
                          <span className="text-sm text-slate-700 dark:text-slate-200 truncate">
                            {p.nombre_producto}{' '}
                            <span className="text-slate-400 dark:text-slate-500">· {p.nombre_negocio}</span>
                          </span>
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6">
          <h5 className="flex items-center gap-2 text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">
          ...
          {cargandoFeed ? (
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500">Todavía no hay productos disponibles.</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {productosFeed.map((p) => (
                <button
                  ...
                  className="group text-left rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden transition hover:border-red-300 hover:shadow-lg hover:-translate-y-0.5"
                >
                  <div className="relative">
                    <img
                  ...
                  <div className="p-3.5">
                    <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm truncate">{p.nombre_producto}</p>
        ...
        <section className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6">
          ...
          {cargandoNegocios ? (
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500">No hay negocios disponibles.</p>
          ) : (
              ...
              {negocios.map((n) => {
                ...
                return (
                  <button
                    ...
                    className="text-left rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden transition hover:border-red-300 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <img
                    ...
                    />
                    <div className="p-4">
                      <p className="font-semibold text-slate-800 dark:text-slate-100">{n.nombre_negocio}</p>
        ...
        {sucursalNavegando && (
          <section className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
              <div>
                <h5 className="flex items-center gap-2 text-lg font-semibold text-slate-800 dark:text-slate-100">
                  <Package size={18} className="text-red-600" />
                  Productos de {sucursalNavegando.nombre}
                </h5>
                {sucursalNavegando.nombreNegocio && (
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{sucursalNavegando.nombreNegocio}</p>
                )}
              </div>
              <button
                onClick={() => {
                ...
                className="text-sm text-slate-500 dark:text-slate-400 hover:text-red-600 transition"
              >
                Cerrar
              </button>
            </div>
            ...
            {cargandoProductosNavegando ? (
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500">Esta sucursal no tiene productos disponibles.</p>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6">
            ...
            {cargandoSucursalesModal ? (
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500">Este negocio no tiene sucursales disponibles.</p>
            ) : (
              <div className="space-y-2">
                {sucursalesModal.map((s) => (
                  <button
                    ...
                    className="w-full text-left rounded-xl border border-slate-200 dark:border-slate-700 hover:border-red-300 hover:bg-red-50/40 transition px-4 py-3"
                  >
                    <p className="font-medium text-slate-800 dark:text-slate-100 text-sm">{s.nombre_sucursal}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{s.direccion}</p>
                  </button>
                ))}
              </div>
            )}
